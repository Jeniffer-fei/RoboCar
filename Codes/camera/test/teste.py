            ...
            for line1 in lines1:
                ...
                for line2 in lines2:
                    ...
                    if distance < 50:  # Limiar para fusão
                        combined_lines.append((line1[0], line2[0]))
...
def calculate_midpoint(line):
    """
    Calcula o ponto médio de uma linha.
    """
    x1, y1, x2, y2 = line
...
def draw_lines(image, lines_left, lines_right, combined_lines):
    ...
    if combined_lines:
        for line1, line2 in combined_lines:
            mid_left = calculate_midpoint(line1)
            mid_right = calculate_midpoint(line2)

            # Desenhar linha horizontal entre os pontos médios
            cv.line(line_image, (int(mid_left[0]), int(mid_left[1])), (int(mid_right[0] + width // 2), int(mid_right[1])), (0, 255, 255), 2)

            # Desenhar linha vertical de 20 cm no meio da linha horizontal
            # Assumindo que 1 cm é aproximadamente 10 pixels, ajuste conforme necessário
            pixel_length = 10  # Substitua com a escala real de cm para pixels
            vertical_length = 20 * pixel_length
            x_mid = (int(mid_left[0]) + int(mid_right[0] + width // 2)) // 2
            y_mid = (int(mid_left[1]) + int(mid_right[1])) // 2
            cv.line(line_image, (x_mid, y_mid - vertical_length // 2), (x_mid, y_mid + vertical_length // 2), (0, 0, 255), 2)
